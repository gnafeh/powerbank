...
//                        String reader = new String(read);
    ...
    private void refreshBatteryInfo() {
        //Log.d("hefang", " ---=== refreshBatteryInfo() ===---");
        ...
        while (it.hasNext()) {
            ...
            if (batteryInfo.getBat_id() != Constant.INVALIDATE_ID) {
                ...
                if ((batteryInfo.getBat_percent() > Constant.VOLTAGE_PERCENT)
                   ...
                   /*
                    Log.d("hefang", " slot id: " + getSlotIndexByBatId(batteryInfo.getBat_id()));
                    Log.d("hefang", " slot id2: " + slot_id);
                    Log.d("hefang", " is locked: " + (batteryInfo.getIs_locked()));
                   */
                    if((getSlotIndexByBatId(batteryInfo.getBat_id()) == slot_id)
                            && (batteryInfo.getIs_locked() == 1)) {
                    }else{
                        usable_count++;
                    }
                }
        ...
        //Log.d("hefang", " empty: " + batteryEmptyCount + ", usable: " + batteryUsableCount);
    }
    ...
    private void parseAndSaveBatteryInfo(byte[] bat_info, int slot_index) {
        //Log.d("hefang", " ---=== parseAndSaveBatteryInfo() ===---");
    ...
    private void getSlotDataFromLocal(String filePath) {
        Log.d("hefang", " ---=== getSlotDataFromLocal() ===---");
        Log.d("hefang", " filePath: " + filePath);
        slotMap = gson.fromJson(readJson(filePath),HashMap.class);

        Log.d("hefang", " slotMap: " + slotMap);
        Log.d("hefang", " readJson(filePath): " + readJson(filePath));
        Log.d("hefang", " HashMap: " + HashMap.class);
        Log.d("hefang", " getSlotDataFromLocal() end ---------------");
    }

    private String getFilePath() {
        Log.d("hefang", " ---=== getFilePath() ===--- ");
        String headerDir;
    ...
    public  String readJson(String path) {
        Log.d("hefang", " ---=== readJson() ===---");
        Log.d("hefang", " path: "+ path);
...
//        return "{\"data\":" + data.toString() + "}";
    ...
    private void dealOrder(Message msg) {
        ...
        Log.d("hefang", " ---=== dealOrder ===--- " );
        Log.d("hefang", " msg: " + msg );
    ...
    private void getSlotIndexAndLockState00( ){
        Log.d("hefang", " ---=== getSlotIndexAndLockState000 ===--- " );
        int i = 0;
        BatteryInfo batteryInfo;
        String slot_str;
        ...
        for (i = 1; i <= batteryCount; i++) {
            ...
            if (batteryMap.containsKey(slot_str)) {
                ...
                if(i==6){
                    Log.d("hefang", " iiiii: "+ i );
                    //batteryInfo.setIs_locked(1);
                }
                Log.d("hefang", " getSlotIndexAndLockState000: "+ batteryInfo.getIs_locked() );
            }
        }
    }
            ...
            public void onSuccess(ResultDesc resultDesc) {
                ...
                if (null != resultDesc) {
                    ...
                    if (null != result) {
                        ...
                        //hefang add start
                        Log.d("hefang", " settingBean: " +settingBean);
                        server_app_ver = getServerVersion(settingBean.getApp_url());
                        Log.d("hefang", " server_app_ver: " +server_app_ver);
                        //Log.d("hefang", "syncSetting  settingBean.getApp_url(): " +settingBean.getApp_url());

                        Log.d("hefang", " needDownloadAndUpgrade: " +needDownloadAndUpgrade );
...
//                        sendTcpLogin();
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        Log.d("hefang", " ---=== onActivityResult() ===---" );
        super.onActivityResult(requestCode, resultCode, data);
        Log.d("hefang", " requestCode: " + requestCode + ",resultCode: " + resultCode);
        if (resultCode == RESULT_OK && requestCode == REQUEST_CODE_INSTALL_PERMISSION) {
            Toast.makeText(this, filePath, Toast.LENGTH_SHORT).show();
            Log.d("hefang", " installapk: onActivityResult()" );
            installApk(filePath);
        }
    }
    ...
    public int getServerVersion(String app_url) {
        Log.d("hefang", " ---=== getServerVersion() ===--- " );
        //hefang get server app version
        int int_ver = 10000;
        Log.d("hefang", " app_url: " +app_url);
...
//            sendTcpMsg();
    ...
    private boolean getLockStateBySlotId(int slot_id){
        Log.d("hefang", " ---=== getLockStateBySlotId ===--- " );
        int i = 0;
        BatteryInfo batteryInfo;
        String slot_str;
        for (i = 1; i <= batteryCount; i++) {
            ...
            if (batteryMap.containsKey(slot_str)) {
                ...
                if(i==slot_id){
                    Toast.makeText(this, "i= "+i+", lock= " +batteryInfo.getIs_locked() , Toast.LENGTH_SHORT).show();
                    return (batteryInfo.getIs_locked() ==1);
                }
                Log.d("hefang", " getSlotIndexAndLockState: "+ batteryInfo.getIs_locked() );
            }
        }
            ...
            public void onReceive(Context context, Intent intent) {
                ...
                switch (action) {
                    ...
                    case Constant.TCP_CMD_LOCK_SLOT:
                        Log.d("hefang", " case:TCP_CMD_LOCK_SLOT:----- " );
                        ...
                        getSlotDataFromLocal(getFilePath());
                        Log.d("hefang", " slotMap: " + slotMap);
                        Log.d("hefang", " getFilePath(): " + getFilePath());
                        Log.d("hefang", " slot_id:" + slot_id);
                    ...
                    case Constant.TCP_CMD_UNLOCK_SLOT:
                        Log.d("hefang", " case:TCP_CMD_UNLOCK_SLOT -------"  );
                        slot_id = intent.getIntExtra(Constant.LOCK_SLOT, 1);
    ...
    private void borrowConfirmAync(final String order_id, final int status, long bat_id) {
        ...
        Log.d("hefang", " ---=== borrowConfirmAync ===---");
        Log.d("hefang", " ssssssslot id: " + slot_index);
        Log.d("hefang", " battttt id: " + getLockStateBySlotId(slot_index));
